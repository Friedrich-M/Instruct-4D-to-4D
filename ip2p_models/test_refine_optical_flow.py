...
ddim_inv_prompt = ""
selected_diffusion_step = 5
selected_num_train_timesteps = 600
guidance_scale = 9.5
...
for batch_index in range(0, dataset_length, sequence_length - overlap_length):
    if batch_index + sequence_length > dataset_length:
        break
    ...
    images_sequence = images_input[batch_index:batch_index+sequence_length] # (sequence_length, c, h, w)
    images_sequence_cond = images_condition[batch_index:batch_index+sequence_length] # (sequence_length, c, h, w)
    
    torchvision.utils.save_image(images_sequence, 'images_sequence.png', nrow=sequence_length, padding=0, pad_value=1)
    torchvision.utils.save_image(images_sequence_cond, 'images_sequence_cond.png', nrow=sequence_length, padding=0, pad_value=1)
    ...
    diffusion_step = 20 if batch_index == 0 else selected_diffusion_step
    num_train_timesteps = 1000 if batch_index == 0 else selected_num_train_timesteps
    latents_type = selected_latents_type
    
    torchvision.utils.save_image(images_sequence, 'images_sequence_warped.png', nrow=sequence_length, padding=0, pad_value=1)
    ...
    torchvision.utils.save_image(images_refine, f'images_refine.png', nrow=sequence_length, padding=0, pad_value=1)
    
    images_input[batch_index:batch_index+sequence_length] = images_refine