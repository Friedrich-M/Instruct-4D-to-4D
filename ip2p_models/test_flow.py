...
def run(args, image1_path, image2_path):
    ...
    with torch.no_grad():
        ...
        Image.fromarray(flow_viz.flow_to_image(flow_fwd)).save('flow_fwd.png')
        Image.fromarray(flow_viz.flow_to_image(flow_bwd)).save('flow_bwd.png')
        ...
        Image.fromarray(image1).save('source_image.png')
        Image.fromarray(image2).save('target_image.png')
        Image.fromarray(edit_image).save('edited_source_image.png')
        
        warp_to_image2 = warp_flow(edit_image, flow_bwd)
        Image.fromarray(warp_to_image2.astype(np.uint8)).save('warp_source_to_target_unmasked.png')
        warp_to_image2 = warp_to_image2 * mask_bwd[:,:,np.newaxis]
        Image.fromarray(warp_to_image2.astype(np.uint8)).save('warp_source_to_target_masked.png')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source_img", type=str, default='./examples/coffee_frame_2x/3.png', help="source image")
    parser.add_argument("--target_img", type=str, default='./examples/coffee_frame_2x/6.png', help="target image")
    args = parser.parse_args()

    run(args, args.source_img, args.target_img)
