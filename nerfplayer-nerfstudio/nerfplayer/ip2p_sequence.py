...
from finetune_ip2p.models.ip2p_pipeline import InstructPix2PixPipeline
from finetune_ip2p.models.ip2p_unet import UNet3DConditionModel
from finetune_ip2p.models.ip2p_utils import ddim_inversion, ddim_inversion_classifier
...
class SequenceInstructPix2Pix(nn.Module):
    ...
    def __init__(self, device: Union[torch.device, str], ip2p_use_full_precision=False,) -> None:
        ...
        CONSOLE.print("Loading Video InstructPix2Pix...")
        self.device = device
        ...
        CONSOLE.print("Video InstructPix2Pix loaded!")
    ...
    def edit_sequence(self,
        ...
        T: int = 1000,
        mean_latent: bool = False,
    ) -> torch.Tensor:
        
        sequence_length, _, H, W = images.shape
        RH, RW = H // 8 * 8, W // 8 * 8
        if RH != H or RW != W: # resize to multiple of 8
        ...
        with torch.no_grad():
            latents = self.imgs_to_latent(images) # (f, 4, h//4, w//4)
            image_latents = self.prepare_image_latents(images_cond) # (f, 4, h//4, w//4)
        ...
        self.ddim_inv_scheduler.config.num_train_timesteps = T
        ...
        if mean_latent:
            latents = latents.mean(dim=0, keepdim=True) # (1, 4, h//4, w//4)
            
        with torch.no_grad():
            edited_images = self.latents_to_img(latents) # (f, 3, h, w)
