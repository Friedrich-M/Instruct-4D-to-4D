...
def run(args, input_path, output_path, output_img_path):
    ...
    with torch.no_grad():
        ...
        for i in range(len(images) - 1):
            print(i)
            image1 = load_image(images[i]) # 当前帧
            image2 = load_image(images[i + 1]) # 下一帧

            padder = InputPadder(image1.shape) 
            image1, image2 = padder.pad(image1, image2) 
            
            _, flow_fwd = model(image1, image2, iters=20, test_mode=True) # 前向光流，从image1到image2的光流，每一个像素点都有一个光流向量，光流向量的大小代表了光流的大小，光流的方向代表了光流的方向
            ...
            mask_fwd, mask_bwd = compute_fwdbwd_mask(flow_fwd, flow_bwd) # 计算前向光流和反向光流的mask，mask的大小和原始图片的大小一致，mask的值为0或者1，0代表不可信，1代表可信，mask的值越大，代表光流的可信度越高，mask的值越小，代表光流的可信度越低
            
            import ipdb; ipdb.set_trace()
            # warp img1 to img2 with flow_fwd
