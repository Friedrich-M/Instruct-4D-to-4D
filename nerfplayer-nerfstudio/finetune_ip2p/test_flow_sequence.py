...
for batch_index in range(0, dataset_length, sequence_length - overlap_length):
    if batch_index + sequence_length > dataset_length:
        break
    images = images_input[batch_index:batch_index+sequence_length] # (sequence_length, c, h, w)
    images_cond = images_condition[batch_index:batch_index+sequence_length] # (sequence_length, c, h, w)
    
    torchvision.utils.save_image(images, 'images_render.png', nrow=sequence_length, padding=0, pad_value=1)
    ...
    torchvision.utils.save_image(images, 'images_input.png', nrow=sequence_length, padding=0, pad_value=1)
    torchvision.utils.save_image(images_cond, 'images_condition.png', nrow=sequence_length, padding=0, pad_value=1)
    ...
    torchvision.utils.save_image(video, f'images_edit.png', nrow=sequence_length, padding=0, pad_value=1)
    
    import ipdb; ipdb.set_trace()
    
    images_input[batch_index+sequence_length-overlap_length:batch_index+sequence_length] = video[-overlap_length:]
