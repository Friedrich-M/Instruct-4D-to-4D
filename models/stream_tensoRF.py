...
class StreamTensorVMSplit(StreamTensorBase):
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        add = lambda x: x+ceil((self.num_frames-1)*self.ld_per_frame)*self.level_dim_multi
        # add = lambda x: x+ceil((self.num_frames-1)*self.ld_per_frame)
        plane_coef, line_coef = [], []
        for i in range(len(self.vecMode)):
            vec_id = self.vecMode[i]
            mat_id_0, mat_id_1 = self.matMode[i]
            plane_coef.append(torch.nn.Parameter(
                scale * torch.randn((1, add(n_component[i]), gridSize[mat_id_1], gridSize[mat_id_0]))))  #
            line_coef.append(
                torch.nn.Parameter(scale * torch.randn((1, add(n_component[i]), gridSize[vec_id], 1))))
    ...
    def feat_diff_loss(self, frame):
        ...
        for idx_plane in range(len(self.density_plane)):
            loss += self.feat_diff(frame, self.density_plane[idx_plane], self.density_list[idx_plane][0])
            loss += self.feat_diff(frame, self.density_line[idx_plane], self.density_list[idx_plane][0])
        for idx_plane in range(len(self.app_plane)):
            loss += self.feat_diff(frame, self.app_plane[idx_plane], self.app_list[idx_plane][0])
            loss += self.feat_diff(frame, self.app_line[idx_plane], self.app_list[idx_plane][0])
        return loss
    ...
    def compute_densityfeature(self, xyz_sampled, frame):
        ...
        for idx_plane in range(len(self.density_plane)):
            plane_coef_point = F.grid_sample(
                self.get_embeds(frame.item(), self.density_plane[idx_plane], *self.density_list[idx_plane]), 
                coordinate_plane[[idx_plane]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
            line_coef_point = F.grid_sample(
                self.get_embeds(frame.item(), self.density_line[idx_plane], *self.density_list[idx_plane]), 
                coordinate_line[[idx_plane]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
    ...
    def compute_appfeature(self, xyz_sampled, frame):
        ...
        for idx_plane in range(len(self.app_plane)):
            plane_coef_point.append(F.grid_sample(
                self.get_embeds(frame.item(), self.app_plane[idx_plane], *self.app_list[idx_plane]), 
                # self.app_plane[idx_plane], 
                coordinate_plane[[idx_plane]], align_corners=True).view(-1, *xyz_sampled.shape[:1]))
            line_coef_point.append(F.grid_sample(
                self.get_embeds(frame.item(), self.app_line[idx_plane], *self.app_list[idx_plane]), 
...
class StreamTensorCP(StreamTensorBase):
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        ...
        for i in range(len(self.vecMode)):
            vec_id = self.vecMode[i]
            line_coef.append(
                torch.nn.Parameter(scale * torch.randn((1, add(n_component), gridSize[vec_id], 1))))
    ...
    def compute_densityfeature(self, xyz_sampled, frame):
        ...
        coordinate_line = torch.stack((torch.zeros_like(coordinate_line), coordinate_line), dim=-1).detach().view(3, -1, 1, 2)
        line_coef_point = F.grid_sample(
            self.get_embeds(frame.item(), self.density_line[0], *self.density_list[0]),
            coordinate_line[[0]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
        line_coef_point = line_coef_point * F.grid_sample(
            self.get_embeds(frame.item(), self.density_line[1], *self.density_list[0]),
            coordinate_line[[1]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
        line_coef_point = line_coef_point * F.grid_sample(
            self.get_embeds(frame.item(), self.density_line[2], *self.density_list[0]),
            coordinate_line[[2]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
    ...
    def compute_appfeature(self, xyz_sampled, frame):
        ...
        coordinate_line = torch.stack((torch.zeros_like(coordinate_line), coordinate_line), dim=-1).detach().view(3, -1, 1, 2)
        line_coef_point = F.grid_sample(
            self.get_embeds(frame.item(), self.app_line[0], *self.app_list[0]),
            coordinate_line[[0]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
        line_coef_point = line_coef_point * F.grid_sample(
            self.get_embeds(frame.item(), self.app_line[1], *self.app_list[0]),
            coordinate_line[[1]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
        line_coef_point = line_coef_point * F.grid_sample(
            self.get_embeds(frame.item(), self.app_line[2], *self.app_list[0]),
            coordinate_line[[2]], align_corners=True).view(-1, *xyz_sampled.shape[:1])
